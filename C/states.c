...
int init_actuators_of_transition_callback(void *data, int argc, char **argv, char **azColName) 
{
	...
	st->adevs[st->adevcount].modul = atoi(argv[0]);
	st->adevs[st->adevcount].chnnl = atoi(argv[1]);
	st->adevs[st->adevcount].devid = atoi(argv[2]);
	st->adevs[st->adevcount].dtype = atoi(argv[3]);
	st->adevs[st->adevcount].value = atoi(argv[4]);  
...
//printf("Opened database successfully\n");
		sprintf(sql, "select devices.modul, devices.chnnl, devices.devid, devices.dtype, agdev.value from agdev inner join devices on devices.modul = agdev.modul and devices.devid = agdev.devid where agdev.devgr = %d;", st->agrid);
...
int init_sensors_of_state_callback(void *data, int argc, char **argv, char **azColName) 
{
	...
	st->sdevs[st->sdevcount].modul = atoi(argv[0]);
	st->sdevs[st->sdevcount].chnnl = atoi(argv[1]);
	st->sdevs[st->sdevcount].devid = atoi(argv[2]);
	st->sdevs[st->sdevcount].dtype = atoi(argv[3]);
	st->sdevs[st->sdevcount].fromval = atoi(argv[4]);  
	st->sdevs[st->sdevcount].toval = atoi(argv[5]); 
...
void init_sensors_of_state(state *st)
{
	...
	{
//printf("Opened database successfully\n");
		sprintf(sql, "select devices.modul, devices.chnnl, devices.devid, devices.dtype, sgdev.fromval, sgdev.toval from sgdev inner join devices on devices.modul = sgdev.modul and devices.devid = sgdev.devid where sgdev.devgr = %d;", st->sgrid);
